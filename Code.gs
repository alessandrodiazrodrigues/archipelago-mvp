// =================== GOOGLE APPS SCRIPT - ARCHIPELAGO DASHBOARD API V8.0 ===================
// Cliente: Guilherme Santoro
// Desenvolvedor: Alessandro Rodrigues
// Data: Marco/2026
// Versao: V8.0 - SISTEMA DE LOGIN POR PERFIL
// BASE: V7.4 com UTI + Reservas completos
// NOVIDADES V8.0:
//    - Tabela SENHAS_PERFIS com 10 senhas (1 admin + 9 enfermeiros)
//    - Endpoint GET ?action=login - valida senha e retorna perfil + hospital
//    - Perfil admin: acesso completo ao sistema
//    - Perfil enfermeiro: acesso restrito ao hospital vinculado (somente reservas)
// NOVIDADES V7.4:
//    - Coluna L (modalidade_contratada) na aba reservas
//    - Campo categoriaEscolhida salvo corretamente nas reservas UTI
// NOVIDADES V7.3:
...
// =================== SENHAS E PERFIS V8.0 ===================
const SENHAS_PERFIS = {
  '1997': { perfil: 'admin',      hospital: null },
  '1111': { perfil: 'enfermeiro', hospital: 'H1' },
  '2222': { perfil: 'enfermeiro', hospital: 'H2' },
  '3333': { perfil: 'enfermeiro', hospital: 'H3' },
  '4444': { perfil: 'enfermeiro', hospital: 'H4' },
  '5555': { perfil: 'enfermeiro', hospital: 'H5' },
  '6666': { perfil: 'enfermeiro', hospital: 'H6' },
  '7777': { perfil: 'enfermeiro', hospital: 'H7' },
  '8888': { perfil: 'enfermeiro', hospital: 'H8' },
  '9999': { perfil: 'enfermeiro', hospital: 'H9' }
};
...
const HOSPITAIS = {
  ...
  'H8': { 
    ...
    contratuais: 13,   // CORRIGIDO 11/JAN/2026
    extras: 9,         // CORRIGIDO 11/JAN/2026
    total: 22,
    linhaInicio: 251,
    tipo: 'hibrido_puro'
  },
...
function doGet(e) {
  try {
    Logger.log('>>> [API V7.4] Requisicao GET: ' + JSON.stringify(e.parameter));
    ...
    Logger.log('!!! [API V7.4] Erro em doGet: ' + error.toString());
    const errorResponse = { 
      ok: false, 
...
function doPost(e) {
  try {
    Logger.log('>>> [API V7.4] Requisicao POST');
    ...
    if (e.postData && e.postData.contents) {
      try {
        ...
        Logger.log('[API V7.4] Payload JSON parseado: ' + Object.keys(params).join(', '));
      } catch (err) {
        Logger.log('!!! [API V7.4] Erro ao parsear JSON: ' + err.toString());
        params = e.parameter || {};
      }
    } else {
      params = e.parameter || {};
    ...
    Logger.log('!!! [API V7.4] Erro em doPost: ' + error.toString());
...
function handleTest() {
  ...
  return {
    ok: true,
    data: {
      message: 'API Archipelago Dashboard V7.4 - Sistema Completo com UTI e Reservas!',
      timestamp: new Date().toISOString(),
      version: '7.4',
      colunas: 76,
      enfermaria: {
      ...
      novidades_v74: [
        'Coluna L (modalidade_contratada) na aba reservas',
        'categoriaEscolhida salvo nas reservas UTI',
...
function handleGetAll() {
  try {
    Logger.log('[API V7.4] Lendo todos os dados...');
    ...
    Logger.log('[API V7.4] ' + dados.length + ' leitos lidos, ' + reservas.length + ' reservas');
    ...
    Logger.log('!!! [API V7.4] Erro em handleGetAll: ' + error.toString());
...
function handleAdmitir(params) {
  try {
    Logger.log('[API V7.4] ===== ADMITINDO PACIENTE (ENFERMARIA) =====');
    ...
    return {
      ok: true,
      data: { 
        message: 'Paciente admitido com sucesso V7.4', 
        hospital: hospital, 
        leito: leito,
        tipo: tipoLeito,
        isUTI: isUTI
    ...
    Logger.log('!!! [API V7.4] Erro em handleAdmitir: ' + error.toString());
    return { ok: false, error: error.toString() };
  }
}
...
function handleAdmitirUTI(params) {
  try {
    Logger.log('[API V7.4] ===== ADMITINDO PACIENTE UTI =====');
    ...
    return {
      ok: true,
      data: { 
        message: 'Paciente UTI admitido com sucesso V7.4', 
        hospital: hospital, 
        leito: leitoNumero,
        linha: linhaIndex,
    ...
    Logger.log('!!! [API V7.4] Erro em handleAdmitirUTI: ' + error.toString());
    return { ok: false, error: error.toString() };
  }
}
...
function handleAtualizarUTI(params) {
  try {
    Logger.log('[API V7.4] ===== ATUALIZANDO PACIENTE UTI =====');
    Logger.log('Hospital: ' + params.hospital + ' | IdentificacaoLeito: ' + params.identificacaoLeito);
    ...
    return {
      ok: true,
      data: { 
        message: 'Paciente UTI atualizado com sucesso V7.4', 
        hospital: hospital, 
        leito: leitoNumero,
        linha: linhaIndex,
        identificacaoLeito: identificacaoLeito,
    ...
    Logger.log('!!! [API V7.4] Erro em handleAtualizarUTI: ' + error.toString());
    return { ok: false, error: error.toString() };
  }
}
...
function handleDarAltaUTI(params) {
  try {
    Logger.log('[API V7.4] ===== DANDO ALTA UTI =====');
    Logger.log('Hospital: ' + params.hospital + ' | IdentificacaoLeito: ' + params.identificacaoLeito);
    ...
    return {
      ok: true,
      data: { 
        message: 'Alta UTI processada com sucesso V7.4', 
        hospital: hospital, 
        leito: leitoNumero,
        linha: linhaIndex,
        tipo: 'UTI'
    ...
    Logger.log('!!! [API V7.4] Erro em handleDarAltaUTI: ' + error.toString());
    return { ok: false, error: error.toString() };
  }
}
...
function handleAtualizar(params) {
  try {
    Logger.log('[API V7.4] Atualizando: ' + params.hospital + '-' + params.leito);
    ...
    // =================== SISTEMA DE LEITOS IRMAOS (apenas enfermaria H2/H4) ===================
    // Sincroniza a identificacao do irmao sempre que identificacaoLeito ou isolamento mudam
    if (!isUTI) {
      const config = HOSPITAIS[hospital];
      ...
      if (config && config.irmaos) {
        ...
        if (leitoIrmao) {
          ...
          if (linhaIrmao !== -1) {
            const statusIrmao = aba.getRange(linhaIrmao, COLUNAS.STATUS + 1).getValue();
            const irmaoEstaVago = (statusIrmao === 'Vago' || statusIrmao === 'vago' || statusIrmao === '');
            
            if (irmaoEstaVago) {
              // Ler o estado atual do leito principal APOS a atualizacao
              const idAtual       = aba.getRange(linhaIndex, COLUNAS.IDENTIFICACAO_LEITO + 1).getValue() || '';
              const isolAtual     = aba.getRange(linhaIndex, COLUNAS.ISOLAMENTO + 1).getValue() || '';
              
              const isolNorm = isolAtual
                .normalize('NFD')
                .replace(/[\u0300-\u036f]/g, '')
                .toLowerCase()
                .trim();
              const temIsolamento = isolNorm &&
                                    !isolNorm.includes('nao isol') &&
                                    isolNorm !== 'nao isolamento';
              
              if (temIsolamento) {
                // Com isolamento: irmao vago nao deve ter identificacao
                const idIrmaoAtual = aba.getRange(linhaIrmao, COLUNAS.IDENTIFICACAO_LEITO + 1).getValue() || '';
                if (idIrmaoAtual) {
                  aba.getRange(linhaIrmao, COLUNAS.IDENTIFICACAO_LEITO + 1).setValue('');
                  Logger.log('[IRMAOS] Isolamento ativo: id do irmao ' + leitoIrmao + ' apagada ("' + idIrmaoAtual + '")');
                }
              } else {
                // Sem isolamento: irmao vago deve ter a identificacao complementar
                if (idAtual && idAtual.trim()) {
                  const partes = idAtual.split('-');
                  ...
                  if (partes.length === 2) {
                    ...
                    if (sufixoIrmao) {
                      const idIrmao = numeroBase + '-' + sufixoIrmao;
                      aba.getRange(linhaIrmao, COLUNAS.IDENTIFICACAO_LEITO + 1).setValue(idIrmao);
                      Logger.log('[IRMAOS] Id do irmao ' + leitoIrmao + ' atualizada para "' + idIrmao + '"');
                    }
                  } else {
                    Logger.log('[IRMAOS] Id "' + idAtual + '" sem formato base-sufixo — irmao nao alterado');
                  }
                } else {
                  // Identificacao foi limpa: limpar tambem no irmao
                  const idIrmaoAtual = aba.getRange(linhaIrmao, COLUNAS.IDENTIFICACAO_LEITO + 1).getValue() || '';
                  if (idIrmaoAtual) {
                    aba.getRange(linhaIrmao, COLUNAS.IDENTIFICACAO_LEITO + 1).setValue('');
                    Logger.log('[IRMAOS] Id principal vazia: id do irmao ' + leitoIrmao + ' apagada ("' + idIrmaoAtual + '")');
                  }
                }
              }
            } else {
              Logger.log('[IRMAOS] Irmao ' + leitoIrmao + ' esta ocupado — sem alteracao na identificacao');
            }
          }
        }
      }
    }
    // =================== FIM SISTEMA DE LEITOS IRMAOS ===================
    
    Logger.log('[API V7.4] Atualizado: ' + hospital + '-' + leito);
    
    return {
      ok: true,
      data: { 
        message: 'Paciente atualizado com sucesso V7.4', 
        hospital: hospital, 
        leito: leito,
        tipo: tipoLeito,
        isUTI: isUTI
    ...
    Logger.log('!!! [API V7.4] Erro em handleAtualizar: ' + error.toString());
    return { ok: false, error: error.toString() };
  }
}
...
function handleDarAlta(params) {
  try {
    Logger.log('[API V7.4] Dando alta: ' + params.hospital + '-' + params.leito);
    ...
    Logger.log('[API V7.4] Alta processada!');
    
    return {
      ok: true,
      data: { 
        message: 'Alta processada com sucesso V7.4', 
        hospital: hospital, 
        leito: leito,
        tipo: tipoLeito
      }
    ...
    Logger.log('!!! [API V7.4] Erro em handleDarAlta: ' + error.toString());
    return { ok: false, error: error.toString() };
  }
}
...
function handleReservar(params) {
  try {
    Logger.log('[API V7.4] ===== CRIANDO RESERVA =====');
    ...
    var categoriaEscolhida = params.categoriaEscolhida || '';
    
    Logger.log('[RESERVA] hospital=' + hospital + ', tipo=' + tipo + ', identificacaoLeito=' + identificacaoLeito + ', matricula=' + matricula + ', categoriaEscolhida=' + categoriaEscolhida);
    ...
    if (atualizarBloqueioExistente && linhaBloqueioExistente) {
      var range = abaReservas.getRange(linhaBloqueioExistente, 1, 1, 12);
      range.setValues([[
        hospital,
        tipo,
        identificacaoLeito,
    ...
    if (identificacaoIrmao) {
      ...
      if (!irmaoJaExiste) {
        ...
        abaReservas.appendRow([
          ...
          ''                           // L: modalidade_contratada (VAZIO para bloqueio)
        ]);
    ...
    Logger.log('!!! [API V7.4] Erro em handleReservar: ' + error.toString());
    return { ok: false, error: error.toString() };
  }
}
...
function handleCancelarReserva(params) {
  try {
    Logger.log('[API V7.4] ===== CANCELANDO RESERVA =====');
    ...
    Logger.log('!!! [API V7.4] Erro em handleCancelarReserva: ' + error.toString());
    return { ok: false, error: error.toString() };
  }
}
...
function handleListarReservas(params) {
  try {
    Logger.log('[API V7.4] ===== LISTANDO RESERVAS =====');
    ...
    Logger.log('!!! [API V7.4] Erro em handleListarReservas: ' + error.toString());
    return { ok: false, error: error.toString() };
  }
}
...
function handleAdmitirComReserva(params) {
  try {
    Logger.log('[API V7.4] ===== ADMITIR COM RESERVA =====');
    ...
    Logger.log('!!! [API V7.4] Erro em handleAdmitirComReserva: ' + error.toString());
    return { ok: false, error: error.toString() };
  }
}
  ...
  try {
    ...
    for (var i = 1; i < dados.length; i++) {
      ...
      reservas.push({
        ...
        categoriaEscolhida: linha[COLUNAS_RESERVAS.MODALIDADE_CONTRATADA] || ''
      });
    }
...
function lerTodosOsDados() {
  try {
    ...
    Logger.log('!!! [API V7.4] Erro ao ler dados: ' + error.toString());
    throw error;
  }
}
...
function onOpen() {
  ...
  ui.createMenu('Archipelago API V7.4')
...
function handleLogin(params) {
  try {
    ...
    Logger.log('[API V8.0] Tentativa de login - senha: ' + senha.substring(0, 2) + '**');

    if (!senha) {
      return { ok: false, error: 'Senha nao informada' };
    }

    var perfil = SENHAS_PERFIS[senha];

    if (!perfil) {
      Logger.log('[API V8.0] Senha incorreta');
      return { ok: false, error: 'Senha incorreta' };
    }

    Logger.log('[API V8.0] Login OK - perfil: ' + perfil.perfil + ' hospital: ' + perfil.hospital);
    ...
    Logger.log('!!! [API V8.0] Erro em handleLogin: ' + error.toString());
    return { ok: false, error: error.toString() };
  }
}
...
// =================== LOG FINAL ===================
Logger.log('===================================================');
Logger.log('[API V8.0] Script carregado!');
Logger.log('===================================================');
...
Logger.log('Aba reservas (12 colunas)');
Logger.log('===================================================');
...
Logger.log('===================================================');
Logger.log('V7.4 NOVIDADES:');
Logger.log('   - Coluna L (modalidade_contratada) na aba reservas');
Logger.log('   - categoriaEscolhida salvo nas reservas UTI');
Logger.log('   - (V7.3) SPICT-BR ativo na UTI');
Logger.log('===================================================');
